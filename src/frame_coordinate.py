...
class FrameCoordinate:
    def __init__(self, x: int, y: int, adjacency_ops: List[Tuple[int,int]] = [(1,0), (0,1), (-1,0), (0,-1)]) -> None:
        self.x = x
        self.y = y
        self.adjaceny_ops = adjacency_ops
        self.cur_op = 0
    ...
    def nextAdjacent(self) -> 'FrameCoordinate':
        if self.cur_op >= len(self.adjaceny_ops):
            return None
        next_x = self.x + self.adjaceny_ops[self.cur_op][0]
        next_y = self.y + self.adjaceny_ops[self.cur_op][1]
        self.cur_op += 1
